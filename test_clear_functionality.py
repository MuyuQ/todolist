...
def check_database_tasks():
    ...
    try:
        ...
        # 获取所有任务
        cursor.execute(
            "SELECT id, title, is_completed, quadrant FROM tasks ORDER BY created_at DESC"
        )
        ...
        # 获取已完成任务
        cursor.execute(
            "SELECT id, title, is_completed, quadrant FROM tasks WHERE is_completed = 1 ORDER BY created_at DESC"
        )
        ...
        if completed_tasks:
            for task in completed_tasks:
                print(
                    f"  - ID: {task[0]}, 标题: '{task[1]}', 完成: {task[2]}, 象限: {task[3]}"
                )

        # 获取未完成任务
        cursor.execute(
            "SELECT id, title, is_completed, quadrant FROM tasks WHERE is_completed = 0 ORDER BY created_at DESC"
        )
